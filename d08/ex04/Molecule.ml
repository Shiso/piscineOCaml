...
class virtual molecule name (atoms:Atom.atom list) =
	object (self)
		...
		method get_atoms : Atom.atom list = _atoms 

		method to_string : string = ((self#get_name)^(String.make (30 - (String.length (self#get_name))) ' ')^":  "^(self#get_formula))

		method equals (item : molecule) = (self#get_name = item#get_name) && (self#get_formula = item#get_formula)
...
class hydrogenGas =
	object
		inherit molecule "Hydrogen Gas" [new Atom.hydrogen; new Atom.hydrogen]
	end
